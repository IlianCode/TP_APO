...
public class TestJeu {
	...
	private static String testCompareTo(Carte carte1, Carte carte2, int expectedValue) {
		...
		if (compare < 0) {
			...
			msg += " a la même valeur que la carte " + carte2 +  "(fichier: " + carte2.getFileName() + ").";
		}
		...
		if (((compare < 0)  && (expectedValue < 0)) ||
			...
			msg += "\n C'est faux ! => Il doit y avoir un problème dans votre méthode compareTo";			
		}
		return msg;
	}
	...
	private static String testEquals(Carte carte1, Carte carte2, boolean expectedValue) {
		...
		msg = "La carte " + carte1 + "(fichier: " + carte1.getFileName() + ")";
		if (compare) {
			msg += " est identique à la carte " + carte2 +  "(fichier: " + carte2.getFileName() + ").";
		} else {
			msg += " est différente de la carte " + carte2 +  "(fichier: " + carte2.getFileName() + ").";
		}

		if (compare == expectedValue) {
			msg += "\n C'est vrai !";
		} else {
			msg += "\n C'est faux ! => Il doit y avoir un problème dans votre méthode equals";			
		}
		return msg;
	}
